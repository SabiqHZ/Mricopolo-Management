...
class DroppingRepository {
  ...
  DroppingRepository({required this.dio, required this.connectivity, required this.syncQueue});

  /// Returns {synced: true, data: ...} if posted immediately, or
  /// {synced: false, client_id: ...} if queued for later sync.
  Future<Map<String, dynamic>> recordDropping(Map<String, dynamic> data) async {
    final online = await connectivity.isOnline();
    if (online) {
      try {
        final response = await dio.post('/droppings', data: data);
        ...
        final clientId = await syncQueue.enqueue('dropping', data);
