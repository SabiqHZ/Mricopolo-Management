...
export default function InvoicesPage() {
  ...
  const loadInvoices = async (nextStoreId = storeId) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Gagal memuat faktur");
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    ...
    const loadInitialData = async () => {
      try {
        const [nextStores, nextInvoices] = await Promise.all([
          fetchStores(token),
          fetchInvoices("", token),
        ]);
        if (active) {
  ...
  const showDetail = async (invoiceId: number) => {
    ...
    try {
      const [invoice, paymentHistory] = await Promise.all([
        ...
        apiFetch(`/payments/invoice/${invoiceId}`, {}, token) as Promise<
          Payment[]
        >,
      ]);
      setSelected(invoice);
      ...
      setError(
        err instanceof Error ? err.message : "Gagal memuat detail faktur",
      );
    } finally {
  ...
  return (
    <section className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold">Faktur dan Pembayaran</h1>
        <p className="mt-1 text-sm text-gray-600">
          Pantau faktur per warung dan catat pembayaran sebagian atau penuh.
        </p>
      </div>
      <form
            ...
            {!loading &&
              invoices.map((invoice) => (
                <tr className="border-t" key={invoice.id}>
                  ...
                  <td className="p-3">{invoice.store_name}</td>
                  <td className="p-3">
                    {formatCurrency(invoice.total_amount)}
                  </td>
                  <td className="p-3">{formatCurrency(invoice.paid_amount)}</td>
                  <td className="p-3">
                    {formatCurrency(invoice.outstanding_balance)}
                  </td>
                  <td className="p-3">{STATUS_LABEL[invoice.status]}</td>
                  <td className="p-3">
                    <button
                      className="text-blue-700"
                      type="button"
                      onClick={() => void showDetail(invoice.id)}
                    >
                      Lihat
                    </button>
                  </td>
      ...
      {detailLoading && <p className="text-sm">Memuat detail faktur...</p>}
      {selected && !detailLoading && (
        <section className="space-y-4 rounded-lg bg-white p-5 shadow-sm">
          <div>
            <h2 className="text-lg font-medium">
              Faktur #{selected.id} · {selected.store_name}
            </h2>
            <p className="mt-1 text-sm text-gray-600">
...
async function fetchInvoices(storeId: string, token: string) {
  const suffix = storeId ? `?store_id=${encodeURIComponent(storeId)}` : "";
  return apiFetch(`/invoices${suffix}`, {}, token) as Promise<Invoice[]>;
}
