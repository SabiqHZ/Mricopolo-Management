...
export default function ProductsPage() {
  ...
  const removeProduct = async (product: Product) => {
    if (
      !token ||
      !window.confirm(
        `Hapus ${product.name}? Riwayat transaksi akan tetap disimpan.`,
      )
    )
      return;
    setError("");
    ...
    <section className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-600">
          Kelola produk yang tersedia untuk penjualan titip jual.
        </p>
      </div>
      ...
      >
        ...
        <Field
          ...
          onChange={(value) => updateField("base_price", value)}
          type="number"
          min="0"
          required
        />
